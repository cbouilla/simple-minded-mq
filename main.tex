...
\begin{abstract}
  ...
  Boolean systems which is essentially a special case of already known more
  sophisticated algorithms. The main idea fits in a single sentence: guess
  enough of variables so that the remaining quadratic equations can be solved by
  linearization (\textit{i.e.} by considering each remaining monomial as an
  independent variable and solving the resulting linear system). Under strong
  heuristic assumptions, this finds all the solutions of $m$ quadratic
  polynomials in $n$ variables with $\bigOsoft{2^{n-\sqrt{2m}}}$
  operations. Although this complexity is worst than the best known algorithm,
  the present method has the advantage of being simpler to describe and easy to
  implement. In strong contrast with the state-of-the-art, it is also quite
...
[Charles] : En fait, en faisant ça, on se retrouve avec un système linéaire à
résoudre de taille $v^3$. On peut éliminer d'avance les terms cubiques, donc il
nous reste quelque chose de taille $v^2$, c'est plus gros qu'avant. En fait, la
vraie technique de Joux-Vitse, c'est d'avoir un système linéaire de taille $v$
seulement. Pour cela, il faut annuler les $\binom{v}{3}$ monômes cubiques en
$z$, les $\binom{v}{2}$ monômes quadratiques en $z$, et aussi les
$u*\binom{v}{2}$ monômes cubiques de la forme $y_i z_j z_k$. Enfin, il faut $v$
equations en rab à la fin pour résoudre. Ceci donnerait donc:

% sage: mon = v + binomial(v, 2) + binomial(v, 3) + (n-v) * binomial(v, 2)
% sage: solve(mon == eqs, v)
