...
The simple idea discussed earlier can be generalized in two different ways.
...
\paragraph{The \textsf{CrossBred} algorithm.}
...
polynomials by monomials of degree $d-2$ (i.e. construct the Macaulay matrix of
degree $d$).
Then, consider the linear space spanned 
by $f_1, \dots, f_m, x_1f_1, \dots x_{n-(d-1)}\dots x_{n}f_m$. 
...
The algorithm enumerate all possible $y_1, \dots, y_u$, solve the system in the $z_i$
variable and check whether the solution is consistent.

It is trivial to see that in the case $d=2$ this is the same as Algorithm~\ref{the-algo}.
Let us see what happens in the case $d=3$. The first step consist in multiplying each of
