...
% We analyze the algorithm under the assumption that no linear combination of the
% input polynomials yields a linear equation. This implies in particular that the
% input polynomials must be linearly independent. In addition, it is easy to get
% rid of:
% \begin{itemize}
% \item If a linear combination of the quadratic polynomials yields zero, then one
%   of the polynomials in the combination can be removed from the system.

% \item If a linear combination of the quadratic polynomials yields one, then the
%   system is inconsistent.

% \item If a linear combination of the quadratic polynomials yields a non-constant
%   linear equation, then this equation can be used to rewrite one variable in
%   terms of the others. This removes one variable and one equation.
% \end{itemize}

% The above procedure can be iterated until the assumption is satisfied. Note that
% it will be satisfied with overwhelming probability by random systems of
% quadratic equations.
...
%Note that there are $v(v-1)/2$ quadratic monomials in $v$.
