...
$\bigO{n^2 2^{0.815n}}$ using $n^2 2^{0.63n}$ bits of memory. Implementing these
algorithms is likely to be difficult and more importantly completely useless

the
exponential space requirements of these algorithm make it quite useless to
...
\TODO{ceci contient l'amélioration car ça simplifie la description.}
...
% \TODO{Discuter ici qu'on peut améliorer la production du système linéaire avec un code de Gray}

% sage: var('v', 'n', 'm')                                   
% sage: eqs = m * (v + 1)                                    
% sage: mon = v + binomial(v, 2) + binomial(v, 3)            
% sage: solve(mon == eqs, v)
% [v == -1/6*(6*m - 5)*(-I*sqrt(3) + 1)/(3*m + 1/3*sqrt(-72*m^3 + 261*m^2 - 150*m + 125/3))^(1/3) - 1/2*(3*m + 1/3*sqrt(-72*m^3 + 261*m^2 - 150*m + 125/3))^(1/3)*(I*sqrt(3) + 1), v == -1/6*(6*m - 5)*(I*sqrt(3) + 1)/(3*m + 1/3*sqrt(-72*m^3 + 261*m^2 - 150*m + 125/3))^(1/3) - 1/2*(3*m + 1/3*sqrt(-72*m^3 + 261*m^2 - 150*m + 125/3))^(1/3)*(-I*sqrt(3) + 1), v == 1/3*(6*m - 5)/(3*m + 1/3*sqrt(-72*m^3 + 261*m^2 - 150*m + 125/3))^(1/3) + (3*m + 1/3*sqrt(-72*m^3 + 261*m^2 - 150*m + 125/3))^(1/3)]

% Affreux, on voudrait simplifier, pour cela on oublie simplement les équations de départ et on ne garde que les multiples

% sage: eqs = m * (v)                                        
% sage: solve(mon == eqs, v)                                 
% [v == -sqrt(6*m - 5), v == sqrt(6*m - 5), v == 0]
...
linear systems modulo 2 (say, $20 \times 10$). The key to obtain good software
performance is \emph{vectorization}.
...
$f$. The $c$ output wire indicates whether the linear system $A z^t = b$ is
\emph{consistent}. When that is the case, a solution can be read on the output
wires for $z$. The $f$ output wire indicates whether the matrix $A$ has
\emph{full-rank}. When $c=0$, the system has no solution ; when $c=1$ and $f=1$,
the system has single solution (which the circuit returns) ; when $c=1$ and
$f=0$, the system has several solutions. A circuit with this specification is
not difficult to obtain by putting $A$ into row-reduced echelon form.
...
which allows to perform Boolean operations on 256-bit registers. High-end
\textsf{x86-64} CPUs have \textsf{AVX512} instructions that operate on 512-bit
registers.

This allows to group the iterations of the main loop if the algorithm by batches
of size $w$. The most common situation is that all linear systems are
inconsistent (which results in $c = 0000 \dots 000$). In this case, there is
nothing to do. Otherwise, the output of each element of the batch must be
examined separately. If $c \wedge \neg f = 000 \dots 000$, then all consistent
systems have a single solution, which is already computed. Otherwise, there is a
consistent linear system with several solutions, which need to be
recomputed. This is fairly rare, so the performance of this special case does
not really matter.
\bibliography{biblio}
